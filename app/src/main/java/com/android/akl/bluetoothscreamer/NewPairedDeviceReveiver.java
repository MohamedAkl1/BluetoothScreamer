    ...
    public void onReceive(Context context, Intent intent) {
        if(Objects.equals(intent.getAction(), BluetoothDevice.ACTION_BOND_STATE_CHANGED)){
            if(intent.getIntExtra(BluetoothDevice.EXTRA_BOND_STATE,BluetoothDevice.ERROR) == BluetoothDevice.BOND_BONDED){
                final SharedPreferences sharedPreferences = context.getSharedPreferences(MainActivity.SELECTED_PAIRED_DEVICES,Context.MODE_PRIVATE);
                Set<String> allDevices = sharedPreferences.getStringSet(MainActivity.ALL_DEVICES, null);
